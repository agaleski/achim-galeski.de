...
if(strlen($name) < 2) {
    if(!$name) {
        $errors[] = "Sie müssen einen Namen eingeben.";
    } else {
        $errors[] = "Ein Name muss aus mindestens 2 Zeichen bestehen.";
    }
}
if(!$email) {
    $errors[] = "Sie müssen eine E-Mail angeben.";
} else if(!validEmail($email)) {
    $errors[] = "Sie müssen eine korrekte E-Mail angeben.";
}
if(strlen($message) < 10) {
    if(!$message) {
        $errors[] = "Sie müssen eine Nachricht eingeben.";
    } else {
        $errors[] = "Eine Nachricht muss aus mindestens 10 Zeichen bestehen.";
    }
}
...
// Send the email
$mj = new Mailjet('1034df5cc332d3b187f0f0a12063ac50','a4b45914f306de3761bef2731f9bca8c');
$params = array(
    "method" => "POST",
    "from" => "achim@achim-galeski.de",
    "to" => "achim.galeski@gmail.com",
    "subject" => "Nachricht über das Kontakformular auf achim-galeski.de",
    "text" => "Nachricht:" . PHP_EOL . $message . PHP_EOL . "Von: " . $email . PHP_EOL . $name//,
);
$result = $mj->sendEmail($params);

if ($mj->_response_code == 200)
    if(array_key_exists("js",$_POST)) {
        die("Ihre Nachricht wurde erfolgreich versendet!");
    } else {
        sleep(2);
        header("Location: http://achim-galeski.de/?contact=1");
    }
else {
    if(array_key_exists("js",$_POST)) {
        die("Es ist ein Fehler bei der Übermittlung der Nachricht aufgetreten. Bitte zu einem späteren Zeitpunkt noch ".
            "einmal versuchen.");
    } else {
        sleep(2);
        header("Location: http://achim-galeski.de/?contact=0&message=" . urlencode($message));
    }
